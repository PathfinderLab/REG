...
class KeywordEvaluator:
    ...
    def get_keywords(self, text: str, min_length: int = 3) -> List[str]:
        ...
        keywords = []
        for ent in doc.ents:
            if len(ent.text) >= min_length:  # 길이 기반 필터링
...
class REG_Evaluator:
    def __init__(self, embedding_model, spacy_model='en_core_sci_lg', api_key=None, stage='public'):
    ...
    def evaluate_text(self, ref_text, hyp_text):
        ...
        if self.stage=='public':
            ranking_score= 0.15*(rouge_score+bleu_score) + 0.4*key_score + 0.3*emb_score
        elif self.stage=='private':
            ranking_score = 0.1*(rouge_score+bleu_score) + 0.3*key_score + 0.2*emb_score
